...
# main function
@ndradexhyperfine.utils.set_defaults(**ndradexhyperfine.config["grid"])
def run(
    query,
    QN_ul,
    T_kin=100,
    ...
    *,
    ...
    with TemporaryDirectory(dir=work_dir) as temp_dir:
        with ndradexhyperfine.db.LAMDA(query, temp_dir) as lamda:
...
def generate_radex_paths(lamda, empty):
    ...
    path = str(ndradexhyperfine.RADEX_BINPATH / "radex-{geom}")
...
def execute(dataset, QN_ul, *iterables, dir=".", progress=True, n_procs=None):
    ...
    with outfile.open("w", buffering=1) as f, ndradexhyperfine.utils.runner(
        n_procs
...
def generate_kwargs(lamda, empty):
    ...
    for args in flatargs:
        ...
        kwargs["id"] = ndradexhyperfine.utils.random_hex()
